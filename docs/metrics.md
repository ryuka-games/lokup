...
```
Level 1: 総合スコア         ← 一瞬で全体の健全性を把握
Level 2: カテゴリスコア     ← 4つのカテゴリの状態を確認
Level 3: 個別メトリクス     ← 各指標の値と診断テキスト
Level 4: エビデンス         ← グラフ・テーブルで根拠を確認
```
...
├──────────────────────────────────────────────────────┤
│ LEVEL 2-4: カテゴリ詳細セクション（×4）               │
│                                                      │
│ ┌ 📈 開発速度 ─ スコア 72 / グレードB ─────────────┐  │
│ │ 診断: PRリードタイムが長く、開発速度が低下しています │  │
│ │                                                  │  │
│ │ ▶ PRリードタイム: 8.2日                          │  │
│ │   └─ 展開すると:                                  │  │
│ │      ├─ 診断テキスト                              │  │
